...
async def upload_knowledge_base(
    ...
    if not safe_file_name or safe_file_name != file_name:
        raise HTTPException(status_code=400, detail="Upload a file with a valid filename.")
    if extension not in ALLOWED_EXTENSIONS:
        raise HTTPException(status_code=415, detail="Only PDF, DOCX, MD, and TXT files are supported.")

    content = await file.read()
    if not content:
        raise HTTPException(status_code=400, detail="The uploaded file is empty.")
    if len(content) > MAX_FILE_SIZE_BYTES:
        raise HTTPException(status_code=413, detail="Knowledge-base files must be 10 MB or smaller.")

    business_exists = await session.scalar(select(User.id).where(User.id == business_id))
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="The document could not be read.") from exc
    ...
    previous_text = document_path.read_text(encoding="utf-8") if document_path.exists() else None
